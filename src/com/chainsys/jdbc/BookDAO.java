...
ArrayList<Book> booklist=new ArrayList<Book>();
	public void addBook(Book book) throws Exception {
		...
		String sql = "insert into books(book_id_seq.nextval(),name,price,publish_date)values(?,?,?,?)";
		PreparedStatement preparedStatement = connection.prepareStatement(sql);
	
		preparedStatement.setString(1,book.name);

		preparedStatement.setFloat(2, book.price);
	...
	public void updateBook(int id, String name) throws Exception {
		...
		Connection connection=ConnectionUtil.getConnection();
		String sql1 = "update movies set name=?,price=? where id=?";
...
Book book=new Book();
	...
	public Book findById(Book book) throws Exception {
		...
		{
		Connection connection=ConnectionUtil.getConnection();
		String sql3 = "select id,name,price from movies where id=?";
